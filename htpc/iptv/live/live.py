...
def updateChannelUrlsM3U(channels, template_channels):
    ...
    with open("./htpc/iptv/live/live.m3u", "w", encoding="utf-8") as f_m3u:
        f_m3u.write(f"""#EXTM3U x-tvg-url={",".join(f'"{epg_url}"' for epg_url in config.epg_urls)}\n""")

        with open("./htpc/iptv/live/live.txt", "w", encoding="utf-8") as f_txt:
            for group in config.announcements:
                    ...
                    for channel_name in channel_list:
                        if channel_name in channels[category]:
                            ...
                            for url in filtered_urls:
                                url_suffix = f"$在线IPV4" if len(filtered_urls) == 1 else f"$在线IPV4『线路{index}』"
                                if '$' in url:
...
if __name__ == "__main__":
    template_file = "./htpc/iptv/live/demo.txt"
    correction_file = "./htpc/iptv/live/correction.txt"
